...
Directly movable timeline positions must expose a clear draggable marker head at the top of the vertical guide. The thin line is visual guidance, not the primary drag target. Playhead/navigation uses restrained blue, loop boundaries green, trim boundaries muted mustard `#9C741F`, and recording uses record red. Color is always reinforced by label/glyph/context. Primary interaction targets must be tablet-friendly (minimum 48 dp) and must not require mouse-like precision. See `TIMELINE_INTERACTION_GUIDELINES.md`.

## D-015 — Transport owns the timeline while active
The Studio transport model is STOPPED / PLAYING / RECORDING; there is no separate pause control. The canonical visible buttons are return-to-start, play→stop, record and loop using familiar symbols rather than text labels. While PLAYING or RECORDING, user edits to playhead, loop, trim, clip placement and import/edit actions are locked. Only the transport engine may advance the execution/record head while active. Play/record controls remain disabled until the real production engine is implemented and gated; fake enabled controls are prohibited.
