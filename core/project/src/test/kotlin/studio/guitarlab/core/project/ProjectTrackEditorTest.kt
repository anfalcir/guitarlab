...
class ProjectTrackEditorTest {
    private fun project(): GuitarProject = GuitarProject(
        ...
        clips = listOf(AudioClip(id = "clip-b", trackId = "b", name = "Take", sourceUri = "managed://media/source/take.wav", startFrame = 0, lengthFrames = 100)),
    )

    @Test fun reorderMovesWholeTrackIdentityAndNormalizesOrder() {
    ...
    @Test fun reorderToEndPreservesClipReferences() {
    ...
    @Test fun reorderSameIndexIsExactNoOp() {
        val original = project()
        assertSame(original, ProjectTrackEditor.reorderTrack(original, "b", 1, 999))
    }

    @Test fun clearTrackContentsPreservesTrackAndOtherLanes() {
        val withOtherClip = project().copy(clips = project().clips + AudioClip(id = "clip-a", trackId = "a", name = "Other", sourceUri = "managed://other.wav", startFrame = 0, lengthFrames = 20))
    ...
    @Test fun rejectsUnknownTrackAndOutOfBoundsTarget() {
